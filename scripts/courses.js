...
document.addEventListener('click', function(event) {
    ...
    if (!nav.contains(event.target) && !document.getElementById('hamburger').contains(event.target)) {
...
function filterCourses(subject) {
    ...
    updateTotalCredits(filteredCourses); // Update total credits whenever courses are filtered
}
...
function updateTotalCredits(coursesList) {
    const totalCredits = coursesList.reduce((total, course) => total + course.credits, 0);
